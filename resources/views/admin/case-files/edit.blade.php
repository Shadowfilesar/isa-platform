    ...
    <div class="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <h1 class="mt-3 text-2xl font-semibold tracking-tight text-white sm:text-3xl">Edit File</h1>
                <p class="mt-2 max-w-3xl text-sm text-slate-400 sm:text-base">
                    Update file information, security controls, classification, and case archive settings.
                </p>
            </div>
        ...
        <div class="mb-6 grid gap-4 lg:grid-cols-3">
            ...
            <div class="rounded-2xl border border-slate-800 bg-slate-900/80 p-5 shadow-2xl shadow-black/20 lg:col-span-2">
                <div class="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">Current File</div>
                <div class="mt-2 text-lg font-semibold text-white">{{ $file->title }}</div>
            </div>
        </div>
                ...
                <div class="space-y-6 xl:col-span-2">
                    <div class="overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/80 shadow-2xl shadow-black/20">
                        <div class="border-b border-slate-800 px-6 py-5">
                            <h2 class="text-lg font-semibold text-white">File Preview</h2>
                            <p class="mt-1 text-sm text-slate-400">
                                Keep the existing file or upload a replacement version.
                            </p>
                        </div>

                        <div class="p-6">
                            <div class="mb-5 rounded-2xl border border-slate-800 bg-slate-950/70 p-5">
                                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                    <div class="min-w-0">
                                        <div class="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">Original Filename</div>
                                        <div class="mt-2 truncate text-base font-semibold text-white">{{ $file->original_name }}</div>
                                        <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-400">
                                            <span>{{ $file->mime_type ?: 'Unknown MIME type' }}</span>
                                            <span>{{ number_format(($file->file_size ?? 0) / 1024, 2) }} KB</span>
                                            <span>{{ strtoupper($file->extension ?: 'file') }}</span>
                                        </div>
                                    </div>
                                    ...
                                    @if($file->file_path)
                                        <a href="{{ asset($file->file_path) }}"
                                           ...
                                           class="inline-flex shrink-0 items-center justify-center rounded-xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-2.5 text-sm font-semibold text-cyan-300 transition hover:border-cyan-400/50 hover:bg-cyan-500/20">
                                            Preview File
                                        </a>
                                    @endif
                                </div>
                            ...
                            <label for="file"
                                   class="group flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-slate-700 bg-slate-950/70 px-6 py-10 text-center transition hover:border-cyan-500/40 hover:bg-cyan-500/5">
                                ...
                                <div class="mt-5 text-base font-semibold text-white">Replace current file</div>
                                <div class="mt-2 text-sm text-slate-400">
                                    Leave empty to preserve the current file.
                                </div>
                        ...
                        <div class="grid gap-6 p-6 md:grid-cols-2">
                            <div>
                                ...
                                <select id="category"
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category }}" @selected(old('category', $file->category) === $category)>{{ ucfirst($category) }}</option>
                                    @endforeach
                                </select>
                                @error('category')
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description"
                                          ...
                                          class="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20">{{ old('description', $file->description) }}</textarea>
                                @error('description')
                                    <div class="mt-2 text-sm text-rose-300">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="required_rank"
                                       name="required_rank"
                                       type="text"
                                       value="{{ old('required_rank', $file->required_rank) }}"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input id="required_clearance"
                                       ...
                                       type="text"
                                       value="{{ old('required_clearance', $file->required_clearance) }}"
                        ...
                        <div class="space-y-5 p-6">
                            ...
                            <label class="flex items-start gap-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4 transition hover:border-slate-700">
                                <input type="checkbox"
                                       name="public"
                                       value="1"
                                       @checked(old('public', $file->public))
                                       class="mt-1 h-4 w-4 rounded border-slate-600 bg-slate-900 text-cyan-500 focus:ring-cyan-500/30">
                                <div>
                            ...
                            <div>
                                ...
                                <input id="unlock_event"
                                       name="unlock_event"
                                       type="text"
                                       value="{{ old('unlock_event', $file->unlock_event) }}"
                    ...
                    <div class="overflow-hidden rounded-3xl border border-slate-800 bg-gradient-to-br from-cyan-500/10 via-slate-900 to-slate-900 shadow-2xl shadow-black/20">
                        <div class="px-6 py-5">
                            ...
                            <p class="mt-1 text-sm text-slate-300">
                                Review your configuration and save the changes to this case file.
                            </p>
                        </div>
